...
def hierarchical_contrastive_loss(z1, z2, alpha=0.3, temporal_unit=0, beta=0.5, f_weight=1, trans_type='dct'):
    loss = torch.tensor(0., device=z1.device)
    d = 0
    while z1.size(1) > 1:
        if alpha != 0:
            loss += alpha * instance_contrastive_loss(z1, z2) + alpha * temporal_contrastive_loss(z1, z2)
        if d >= temporal_unit:
            if 1 - alpha != 0:
